var grid = $("#dataTable").bootstrapTable({
    url: "//itech-admin-api.iskwen.com/api/express/listExpressIntegral",
    ...
    exportOptions: {
        fileName: 'listExpressIntegral' + "_" + getFileName(),  //文件名称设置
        worksheetName: 'sheet1',  //表格工作区名称
        tableName: 'listExpressIntegral',
    ...
    columns: [{
        ...
        formatter: function (value, row, index) {
            if (value === 1 ) {
                return "<button type='button' class='btn btn-primary btn-rounded btn-xs ly2-tips' ly2-tips-text='一切正常~'>正常</button>";
            }else{
                return "<button type='button' class='btn btn-danger btn-rounded btn-xs ly2-tips' ly2-tips-text='已被凍結!'>凍結</button>";
            }
        }
    }, {
        field: 'add_time',
...
function deleteOne(id, deleted) {
    ...
    showConfirm("溫馨提示", content, function (index) {
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/express/deleteExpressIntegral",
            type: "post",
            dataType: "json",
...
function batchDelete(deleted) {
    ...
    showConfirm("溫馨提示", content, function (index) {
        ...
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/express/batchDeleteExpressIntegral",
            type: "post",
            dataType: "json",
...
function changeState(id, state) {
    $.ajax({
        url: "//itech-admin-api.iskwen.com/api/express/changeExpressIntegralState",
        type: "post",
        dataType: "json",
...
function batchUpdateState(state) {
    ...
    let content = "";
    if (state === 1) {
        content = "您確定要激活選中的" + rows.length + "個記錄嗎？";
    } else if (state === 2) {
    ...
    showConfirm("溫馨提示", content, function (index) {
        ...
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/express/batchUpdateExpressIntegralState",
            type: "post",
            dataType: "json",
