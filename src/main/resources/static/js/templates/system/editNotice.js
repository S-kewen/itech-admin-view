...
$(document).ready(function () {
    $.ajax({
        url: "//itech-admin-api.iskwen.com/api/system/getNoticeInfo",
        type: "post",
        dataType: "json",
...
function post() {
    ...
    $.ajax({
        url: "//itech-admin-api.iskwen.com/api/system/updateNotice",
        type: "post",
        dataType: "json",
        headers: {'Authorization': getCookie("token")},
        data: "id="+id+"&title=" + title + "&top=" + top +"&startTime=" + startTime+"&endTime="+endTime+"&layText="+layText+"&jumpType="+jumpType+"&url="+url+"&content="+content,
        success: function (msg) {
            if (msg.status === 0) {
