var grid = $("#dataTable").bootstrapTable({
    url: "//itech-admin-api.iskwen.com/api/system/listMessageBoard",
    ...
    exportOptions: {
        fileName: 'listMessageBoard' + "_" + getFileName(),  //文件名称设置
        worksheetName: 'sheet1',  //表格工作区名称
        tableName: 'listMessageBoard',
    ...
    columns: [{
        ...
        formatter: function (value, row, index) {
            if (value == 1) {
                return "<button class='btn btn-primary btn-rounded btn-xs ly2-tips' ly2-tips-text='未匿名~' ly2-tips-color='#23C6C9'>正常</a>";
            } else if (value == 2) {
                return "<button class='btn btn-danger btn-rounded btn-xs ly2-tips' ly2-tips-text='注意保護用戶隱私哦~' ly2-tips-color='#F8AC59'>匿名</a>";
            } else {
        ...
        formatter: function (value, row, index) {
            if (value == 1) {
                return "<button class='btn btn-primary btn-rounded btn-xs ly2-tips' ly2-tips-text='正常~' ly2-tips-color='#23C6C9'>未置頂</a>";
            } else if (value == 2) {
                return "<button class='btn btn-danger btn-rounded btn-xs ly2-tips' ly2-tips-text='置頂~' ly2-tips-color='#F8AC59'>已置頂</a>";
            } else {
                return "<button class='btn btn-default btn-rounded btn-xs ly2-tips' ly2-tips-text='系統一臉懵逼的搖了搖頭~' ly2-tips-color='#FFB800'>未知</a>";
            }
        }
    ...
    },
        {
            ...
            formatter: function (value, row, index) {
                var result = "";
                if (row.state === 1) {
                    result += "<button class='layui-btn layui-btn-xs layui-btn-primary' onClick='javascript:changeState(" + row.id + ",2)'>置頂</button>";
                } else if (row.state === 2) {
                    result += "<button class='layui-btn layui-btn-xs layui-btn' onClick='javascript:changeState(" + row.id + ",1)'>取消</button>";
                }else {
...
function deleteOne(id, deleted) {
    ...
    showConfirm("溫馨提示", content, function (index) {
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/system/deleteMessageBoard",
            type: "post",
            dataType: "json",
...
function batchDelete(deleted) {
    ...
    showConfirm("溫馨提示", content, function (index) {
        ...
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/system/batchMessageBoard",
            type: "post",
            dataType: "json",
...
function changeState(id, state) {
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/system/changeMessageBoardState",
            type: "post",
            dataType: "json",
...
function batchUpdateState(state) {
    ...
    showConfirm("溫馨提示", content, function (index) {
        ...
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/system/batchUpdateMessageBoardState",
            type: "post",
            dataType: "json",
