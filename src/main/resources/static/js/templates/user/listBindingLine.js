var grid = $("#dataTable").bootstrapTable({
    url: "//itech-admin-api.iskwen.com/api/user/listBindingLine",
    ...
    exportOptions: {
        fileName: 'listBindingLine' + "_" + getFileName(),  //文件名称设置
        worksheetName: 'sheet1',  //表格工作区名称
        tableName: 'listBindingLine',
    ...
    columns: [{
        ...
        formatter: function (value, row, index) {
            let utouId="";
            if(value.length>20){
                utouId=value.substr(0,20)+"...";
            }else{
                utouId=value;
            }
            return "<button type='button' class='btn btn-warning btn-rounded btn-xs ly2-tips' ly2-tips-text='"+utouId+"' data-clipboard-text='" + value + "'>複製</button>";
        }
    }, {
        field: 'line_user_id',
        title: 'lineId',
        ...
        formatter: function (value, row, index) {
            if (value == '' || value == undefined) {
                ...
                return "<span class='ly2-tips' ly2-tips-text='"+row.browser+"'>"+value+"</span>";
            }
        }
    }, {
        field: 'state',
        ...
        formatter: function (value, row, index) {
            if (value == 1) {
                ...
                return "<button class='btn btn-danger btn-rounded btn-xs ly2-tips' ly2-tips-text='凍結!' ly2-tips-color='#F8AC59'>凍結</a>";
            } else {
    ...
    },
        {
            ...
            formatter: function (value, row, index) {
                var result = "";
                if (row.state === 1) {
                    result += "<button class='layui-btn layui-btn-xs layui-btn-primary' onClick='javascript:changeState(" + row.id + ",2)'>凍結</button>";
                } else if (row.state === 2) {
                ...
                result+="<button class='layui-btn layui-btn-xs layui-btn-warm'  onClick='javascript:editBindingLine(" + row.id + ",\""+row.line_user_id+"\")'>修改</button>";
                if (row.deleted) {
...
function deleteOne(id, deleted) {
    ...
    showConfirm("溫馨提示", content, function (index) {
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/user/deleteBindingLine",
            type: "post",
            dataType: "json",
...
function batchDelete(deleted) {
    ...
    showConfirm("溫馨提示", content, function (index) {
        ...
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/user/batchDeleteBindingLine",
            type: "post",
            dataType: "json",
...
function changeState(id, state) {
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/user/changeBindingLineState",
            type: "post",
            dataType: "json",
...
function batchUpdateState(state) {
    ...
    showConfirm("溫馨提示", content, function (index) {
        ...
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/user/batchUpdateBindingLineState",
            type: "post",
            dataType: "json",
...
function editBindingLine(id,oldVal) {
    layer.prompt({
        title: '修改',
        value: oldVal
    }, function (val, index) {
        ...
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/user/updateBindingLine",
            type: "post",
            dataType: "json",
            data: "id=" + id + "&val=" + val,
            headers: {'Authorization': getCookie("token")},
