var grid = $("#dataTable").bootstrapTable({
    url: "//itech-admin-api.iskwen.com/api/user/listMessage",
    ...
    columns: [{
        ...
        formatter: function (value, row, index) {
            if (value == 1) {
                return "<button class='btn btn-danger btn-rounded btn-xs ly2-tips' ly2-tips-text='還沒讀~' ly2-tips-color='#1E9FFF'>未讀</a>";
            } else if (value == 2) {
                return "<button class='btn btn-primary btn-rounded btn-xs ly2-tips' ly2-tips-text='已讀~' ly2-tips-color='#009688'>已讀</a>";
            } else {
    ...
    },
        {
            ...
            formatter: function (value, row, index) {
                if (!row.deleted) {
                    return "<button class='layui-btn layui-btn-xs layui-btn-danger' onClick='javascript:deleteMessage(" + row.id + ",\"true\")'>刪除</button>";
                } else {
                    return "<button class='layui-btn layui-btn-xs layui-btn-normal' onClick='javascript:deleteMessage(" + row.id + ",\"false\")'>恢復</button>";
                }
            }
...
function deleteMessage(id, deleted) {
    if (id == '') {
        showTip("請選擇一條記錄");
    } else {
        let content = "";
        ...
        showConfirm("溫馨提示", content, function (index) {
            $.ajax({
                url: "//itech-admin-api.iskwen.com/api/user/deleteMessage",
                type: "post",
                dataType: "json",
...
function batchDelete(deleted) {
    ...
    showConfirm("溫馨提示", content, function (index) {
        ...
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/user/batchDeleteMessage",
            type: "post",
            dataType: "json",
