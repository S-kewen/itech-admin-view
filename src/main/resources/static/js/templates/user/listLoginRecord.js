var grid = $("#dataTable").bootstrapTable({
    url: "//itech-admin-api.iskwen.com/api/user/listLoginRecord",
    ...
    columns: [{
    ...
    },
        {
            ...
            formatter: function (value, row, index) {
                if (!row.deleted) {
                    return "<button class='layui-btn layui-btn-xs layui-btn-danger' onClick='javascript:deleteLoginRecord(" + row.id + ",\"true\")'>刪除</button>";
                } else {
                    return "<button class='layui-btn layui-btn-xs layui-btn-normal' onClick='javascript:deleteLoginRecord(" + row.id + ",\"false\")'>恢復</button>";
                }
            }
...
function deleteLoginRecord(id, deleted) {
    if (id == '') {
        showTip("請選擇一條記錄");
    } else {
        let content = "";
        ...
        showConfirm("溫馨提示", content, function (index) {
            $.ajax({
                url: "//itech-admin-api.iskwen.com/api/user/deleteLoginRecord",
                type: "post",
                dataType: "json",
...
function batchDelete(deleted) {
    ...
    showConfirm("溫馨提示", content, function (index) {
        ...
        $.ajax({
            url: "//itech-admin-api.iskwen.com/api/user/batchDeleteLoginRecord",
            type: "post",
            dataType: "json",
